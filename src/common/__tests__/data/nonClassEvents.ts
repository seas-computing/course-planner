/**
 * Data Science Reading Group
 *
 * Non-class parent for data science reading group formatted to match
 * [[NonClassMeetingResponseDTO]].
 */
export const dataScienceReadingGroup = {
  id: '9548f85e-d613-48d2-94fe-63dbaf95084c',
  contact: 'Jim Waldo',
  title: 'Data Science Reading Group',
  courseId: '0a69ab1e-e741-4215-9c97-ab07575fede5',
  course: {
    id: '0a69ab1e-e741-4215-9c97-ab07575fede5',
    title: 'Data Science',
    area: 'ACS',
    isUndergraduate: false,
    catalogNumber: 'AC 209',
    sameAs: 'CS 109, STATS 121',
    isSEAS: 'Y',
    notes: 'Same as CS 109, STATS 121',
    termPattern: 'FALL',
  },
  spring: {
    id: '59d737ac-aadc-4e67-8d7a-7db2299d0026',
    academicYear: '2020',
    term: 'SPRING',
    nonClassParentId: '9548f85e-d613-48d2-94fe-63dbaf95084c',
    meetings: [
      {
        id: '06369a12-24f0-4a5c-8ecc-9e0124179f75',
        day: 'FRI',
        startTime: '12:14 AM',
        endTime: '12:17 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
      {
        id: '968bd796-0cfb-402a-91e3-59b44c2c9ab9',
        day: 'MON',
        startTime: '12:08 AM',
        endTime: '12:10 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
    ],
  },
  fall: {
    id: '613778f2-ef61-475e-8ca3-71a6416de9a4',
    academicYear: '2020',
    term: 'FALL',
    nonClassParentId: '9548f85e-d613-48d2-94fe-63dbaf95084c',
    meetings: [
      {
        id: 'bd572431-e00e-4a78-9975-1aa90d86c15b',
        day: 'FRI',
        startTime: '12:14 AM',
        endTime: '12:17 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
      {
        id: 'afa76436-4523-4f3b-a952-95ec76c1294d',
        day: 'MON',
        startTime: '12:08 AM',
        endTime: '12:10 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
    ],
  },
};

/**
 * Data Science Reading Group
 *
 * Non-class parent for data science reading group formatted to match
 * [[NonClassMeetingResponseDTO]].
 */
export const computationalModelingofFluidsReadingGroup = {
  id: 'ad9a6a86-4cd1-4063-bd6d-c7379f10c540',
  contact: 'Jim Waldo',
  title: 'Computational Modeling of Fluids and Soft Matter Reading Group',
  courseId: '89bb7cf5-73b8-40d1-87fe-01d8e8ee1cf0',
  course: {
    id: '89bb7cf5-73b8-40d1-87fe-01d8e8ee1cf0',
    title: 'Computational Modeling of Fluids and Soft Matter',
    area: 'ACS',
    isUndergraduate: false,
    catalogNumber: 'AP 274',
    sameAs: 'AC 227',
    isSEAS: 'Y',
    notes: null,
    termPattern: null,
  },
  spring: {
    id: '3086f1be-a15f-491f-b82b-e991f3e747b1',
    academicYear: '2020',
    term: 'SPRING',
    nonClassParentId: 'ad9a6a86-4cd1-4063-bd6d-c7379f10c540',
    meetings: [
      {
        id: '69c72509-3cf0-431a-a1d1-924dcdd43b01',
        day: 'FRI',
        startTime: '12:14 AM',
        endTime: '12:17 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
      {
        id: 'afef329c-90af-4d67-9f86-74568a466d26',
        day: 'MON',
        startTime: '12:08 AM',
        endTime: '12:10 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
    ],
  },
  fall: {
    id: 'dfd31de2-d794-457a-bf46-908618afb7ca',
    academicYear: '2020',
    term: 'FALL',
    nonClassParentId: 'ad9a6a86-4cd1-4063-bd6d-c7379f10c540',
    meetings: [
      {
        id: '8bf44cd9-6aaa-4e39-a295-65794daf45cf',
        day: 'FRI',
        startTime: '12:14 AM',
        endTime: '12:17 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
      {
        id: 'ac220eec-6775-4974-9950-700a2459979e',
        day: 'MON',
        startTime: '12:08 AM',
        endTime: '12:10 AM',
        room: {
          id: '17d19e35-617d-486d-9b40-f30197342c18',
          name: 'Maxwell Dworkin G115 Lecture Theatre',
          campus: 'Cambridge',
        },
      },
    ],
  },
};
