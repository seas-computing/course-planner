 ...
 * tab.
 */
export class FacultyScheduleViews1588102375325 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('CREATE VIEW "FacultyScheduleAbsenceView" AS SELECT "absence"."id" AS "id", absence."type" AS "type", absence."semesterId" AS "semesterId", absence."facultyId" AS "facultyId" FROM "absence" "absence"', undefined);
    await queryRunner.query('INSERT INTO "typeorm_metadata"("type", "schema", "name", "value") VALUES ($1, $2, $3, $4)', ['VIEW', 'public', 'FacultyScheduleAbsenceView', 'SELECT "absence"."id" AS "id", absence."type" AS "type", absence."semesterId" AS "semesterId", absence."facultyId" AS "facultyId" FROM "absence" "absence"']);
    await queryRunner.query('CREATE VIEW "FacultyScheduleCourseView" AS SELECT fci."courseInstanceId" AS "courseInstanceId", fci."facultyId" AS "facultyId", ci."semesterId" AS "semesterId", ci."courseId" AS "courseId", CONCAT_WS(\' \', "c"."prefix", "c"."number") AS "catalogNumber" FROM "faculty_course_instances_course_instance" "fci" LEFT JOIN "course_instance" "ci" ON "ci"."id" = fci."courseInstanceId"  LEFT JOIN "course" "c" ON ci."courseId" = "c"."id"', undefined);
    await queryRunner.query('INSERT INTO "typeorm_metadata"("type", "schema", "name", "value") VALUES ($1, $2, $3, $4)', ['VIEW', 'public', 'FacultyScheduleCourseView', "SELECT fci.\"courseInstanceId\" AS \"courseInstanceId\", fci.\"facultyId\" AS \"facultyId\", ci.\"semesterId\" AS \"semesterId\", ci.\"courseId\" AS \"courseId\", CONCAT_WS(' ', \"c\".\"prefix\", \"c\".\"number\") AS \"catalogNumber\" FROM \"faculty_course_instances_course_instance\" \"fci\" LEFT JOIN \"course_instance\" \"ci\" ON \"ci\".\"id\" = fci.\"courseInstanceId\"  LEFT JOIN \"course\" \"c\" ON ci.\"courseId\" = \"c\".\"id\""]);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('DELETE FROM "typeorm_metadata" WHERE "type" = \'VIEW\' AND "schema" = $1 AND "name" = $2', ['public', 'FacultyScheduleAbsenceView']);
    await queryRunner.query('DROP VIEW "FacultyScheduleAbsenceView"', undefined);
  }
}
